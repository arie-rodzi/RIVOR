# Updated version of rivor_app.py to handle column names in the second row (header=1)
rivor_app_fixed_code = """
...
def compute_rivor_scores(norm_df, weights=None, v=0.5):
    ...
    Qi = v * (1 - (Si - S_best) / (S_worst - S_best + 1e-9)) + \
         (1 - v) * (1 - (Ri - R_best) / (R_worst - R_best + 1e-9))
...
# Save the fixed Streamlit app script
with open("/mnt/data/rivor_app_header2.py", "w") as f:
    f.write(rivor_app_fixed_code)
